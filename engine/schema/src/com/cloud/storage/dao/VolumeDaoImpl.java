...
public class VolumeDaoImpl extends GenericDaoBase<VolumeVO, Long> implements VolumeDao {
    ...
    public List<VolumeVO> listVolumesToBeDestroyed() {
    	SearchBuilder<VolumeVO> delSearch = createSearchBuilder();
    	delSearch.and("state", delSearch.entity().getState(), Op.IN);
        SearchCriteria<VolumeVO> sc = delSearch.create();
        sc.setParameters("state", Volume.State.Destroy,Volume.State.Expunged,Volume.State.Expunging);
