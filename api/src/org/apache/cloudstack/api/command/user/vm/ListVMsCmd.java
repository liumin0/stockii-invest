...
public class ListVMsCmd extends BaseListTaggedResourcesCmd {
    ...
    @Parameter(name = ApiConstants.VOLUME_ID, type = CommandType.UUID, entityType = VolumeResponse.class, description = "list vms by volume. Attached the volume's vms, need params include : volumeId,shareVolume,storageId,accountId/projectId.")
    private Long volumeId;

	@Parameter(name="volumeshare", type=CommandType.BOOLEAN,
            description="list by network volume; true if need to list vms attached the volume, false otherwise")
    private Boolean volumeShare;
    ...
    @Override
    public void execute(){/*
    	if((getVolumeId()!=null && getVolumeShare()==null) || (getVolumeId()==null && getVolumeShare()!=null)){
    		throw new InvalidParameterValueException("The parameter volume and share must appear together!");
    	}
