...
public class AffinityGroupServiceImpl extends ManagerBase implements AffinityGroupService, Manager,
    ...
    public AffinityGroup createAffinityGroupInternal(String account, Long domainId, String affinityGroupName,
        ...
        if (account != null && domainId != null) {
        	Account accountVO = _accountMgr.getActiveAccountByName(account, domainId);
        	if(accountVO.getType()==Account.ACCOUNT_TYPE_PROJECT){
        		ProjectVO p = _projectDao.findByProjectAccountId(accountVO.getAccountId());
        		if(p!=null){
        			owner = _accountMgr.finalizeOwner(caller, null, domainId, p.getId());
        		}else{
