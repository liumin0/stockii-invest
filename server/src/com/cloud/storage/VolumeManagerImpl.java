...
import org.apache.cloudstack.storage.command.AttachShareCommand;
...
public class VolumeManagerImpl extends ManagerBase implements VolumeManager {
    ...
    protected VolumeInfo createVolumeFromSnapshot(VolumeVO volume,
            SnapshotVO snapshot, boolean isOverWrite) throws StorageUnavailableException {
        ...
        try {
         ...
         // swap the info of volume
            if (isOverWrite) {
            	Transaction txn = Transaction.currentTxn();
                txn.start();
                VolumeVO createVolume = _volsDao.findById(result.getVolume().getId());
                VolumeVO oldVolume = _volsDao.findByIdIncludingRemoved(snapshot.getVolumeId());
                if (oldVolume != null && oldVolume.getState() != Volume.State.Destroy) {
                	s_logger.debug("old Volume :"+oldVolume.getId()+"; "+oldVolume.getPath()+"; "+oldVolume.getPodId()+"; "+oldVolume.getPoolType()+"; "
                			+oldVolume.getFolder()+";"+oldVolume.getPoolId()+createVolume.getId());
                	String path = createVolume.getPath();
                	Long podId = createVolume.getPodId();
                	createVolume.setUpdated(new Date());
                	createVolume.setInstanceId(null);
                	createVolume.setState(Volume.State.Destroy);
                	createVolume.setPodId(oldVolume.getPodId());
                	createVolume.setPath(oldVolume.getPath());
                	createVolume.setPoolId(oldVolume.getPoolId());
                	createVolume.setPoolType(oldVolume.getPoolType());
                	createVolume.setFolder(oldVolume.getFolder());
                	createVolume.setDomainId(oldVolume.getDomainId());
                	_volsDao.update(createVolume.getId(), createVolume);
                	
                	oldVolume.setUpdated(new Date());
                	oldVolume.setState(Volume.State.Ready);
                	oldVolume.setPodId(podId);
                	oldVolume.setPath(path);
                	oldVolume.setPoolId(pool.getId());
                	oldVolume.setPoolType(pool.getPoolType());
                	oldVolume.setFolder(pool.getPath());
                	oldVolume.setDomainId(oldVolume.getDomainId());
                	_volsDao.update(oldVolume.getId(), oldVolume);
                	createVolume = oldVolume;
                	txn.commit();
                	
                	return volFactory.getVolume(createVolume.getId(), store);
                } 
            }
    ...
    protected DiskProfile createDiskCharacteristics(VolumeInfo volume,
        ...
        if (volume.getVolumeType() == Type.ROOT
            ...
            // fix creating customized disk with wrong size when another volume is attaching. [qsh, 2013.11.29]
            return new DiskProfile(volume.getId(), volume.getVolumeType(),
                    volume.getName(), diskOffering.getId(),
                    volume.getSize(), diskOffering.getTagsArray(),
                    diskOffering.getUseLocalStorage(),
                    diskOffering.isRecreatable(), null);
        }
    }

    protected VolumeVO createVolumeFromSnapshot(VolumeVO volume, long snapshotId, boolean isOverWrite) throws StorageUnavailableException {
...
//        }
    ...
    public VolumeVO allocVolume(CreateVolumeCmd cmd)
        ...
        if (cmd.getIsOverWrite() && parentVolume != null) {
            parentVolume.setState(Volume.State.Allocated);
            parentVolume.setName(userSpecifiedName);
            volume = new VolumeVO(parentVolume);
        } else {
        	volume = new VolumeVO(userSpecifiedName, -1, -1, -1, -1,
                    new Long(-1), null, null, 0, Volume.Type.DATADISK);
            volume.setPoolId(null);
            volume.setDataCenterId(zoneId);
            volume.setPodId(null);
            ...
            if(domainId==null){
            	domainId = ((caller == null) ? Domain.ROOT_DOMAIN : caller.getDomainId());
            }
        ...
        // Increment resource count during allocation; if actual creation fails,
        // decrement it
        if (parentVolume == null || !cmd.getIsOverWrite()) {
        	_resourceLimitMgr.incrementResourceCount(volume.getAccountId(),
                    ResourceType.volume);
    ...
    public VolumeVO createVolume(CreateVolumeCmd cmd) {
        ...
        try {
            if (cmd.getSnapshotId() != null) {
                volume = createVolumeFromSnapshot(volume, cmd.getSnapshotId(), cmd.getIsOverWrite());
    ...
    public VolumeVO resizeVolume(ResizeVolumeCmd cmd)
        ...
        if (volume.getFormat() == ImageFormat.LUN) {
        	throw new InvalidParameterValueException("Lun device cannot be used for volume resize.");
        }
...
//        }
        if (currentSize > newSize && (_volsDao.getHypervisorType(volume.getId()) == HypervisorType.XenServer
                || _volsDao.getHypervisorType(volume.getId()) == HypervisorType.VMware)) {
        	s_logger.debug("Can not reduce the size of volume for VMware and XenServer.");
        	throw new InvalidParameterValueException("Can not reduce the size of volume for VMware and XenServer.");
        }
        ...
        if (storeForRootStoreScope.getScopeType() != storeForDataStoreScope.getScopeType()) {
            if (storeForDataStoreScope.getScopeType() == ScopeType.CLUSTER) {
                ...
                if (storeForRootStoreScope.getScopeType() == ScopeType.HOST) {
                	...
                	VMInstanceVO  vm = _vmInstanceDao.findById(rootVolumeOfVm.getInstanceId());
                	Long hostId = null;
                	if(vm != null){
...
//                Long hostId = _vmInstanceDao.findById(rootVolumeOfVm.getInstanceId()).getHostId();
    ...
    private VolumeVO sendAttachVolumeCommand(UserVmVO vm, VolumeVO volumeToAttach, Long deviceId, boolean isShared) {
        ...
        if (sendCommand) {
            ...
            //share vmdisk
            if(isShared){
            	AttachShareCommand shareCmd = new AttachShareCommand(disk, vm.getInstanceName(), isShared);
                shareCmd.setManaged(volumeToAttachStoragePool.isManaged());

                shareCmd.setStorageHost(volumeToAttachStoragePool.getHostAddress());
                shareCmd.setStoragePort(volumeToAttachStoragePool.getPort());

                shareCmd.set_iScsiName(volumeToAttach.get_iScsiName());
                ...
                if (chapInfo != null) {
                	shareCmd.setChapInitiatorUsername(chapInfo.getInitiatorUsername());
                	shareCmd.setChapInitiatorPassword(chapInfo.getInitiatorSecret());
                	shareCmd.setChapTargetUsername(chapInfo.getTargetUsername());
                	shareCmd.setChapTargetPassword(chapInfo.getTargetSecret());
                }
                
                shareCmd.setIsShared(isShared);
                
                try {
                	answer = (AttachAnswer)_agentMgr.send(hostId, shareCmd);
                } catch (Exception e) {
        ...
        if (!sendCommand || (answer != null && answer.getResult())) {
        	List<VolumeDetailVO> results = _volumeDetailsDao.findShareDetailByLike(volumeToAttach.getId());
        	if(results.isEmpty() || !_volumeDetailsDao.isSharedDisk(volumeToAttach.getId())){
        	...
        	if(isShared){
        		long volumeId = volumeToAttach.getId();
        		long vmId = vm.getId();
        		String name = "instance-" + vmId;
        		Map<String, String> detail = new HashMap<String, String>();
        		detail.put(name, String.valueOf(vmId));
        		
        		_volumeDetailsDao.persist(volumeId, detail);
        	}
    ...
    public Volume attachVolumeToVM(AttachVolumeCmd command) {
        ...
        boolean isShared = command.isShared();
        Account caller = UserContext.current().getCaller();
        ...
        if (volume.getInstanceId() != null && !command.isShared()) {
            throw new InvalidParameterValueException(
                    "Please specify the volume that is not attached to any VM if the volume is not shared.");
        }
        
        //if the volume is share, it is able to attached to other VM; or it is not attached to any VM.
        VolumeDetailVO detail = _volDetailDao.findDetail(volumeId, "isshared");
        if(detail != null && ((detail.getValue().equals("enabled") && !command.isShared()) || (detail.getValue().equals("disabled") && command.isShared()))){
        	throw new InvalidParameterValueException(
                    "Please specify the volume that is not attached to any VM if the volume is created by unshared, " +
                    "or the volume that is not attached to other VM by unshared if the volume is created by shared");
        }
        
        //this vm and all other attached vm must be stopped
        if((detail != null && (detail.getValue().equals("enabled"))) || (command.isShared())){
        	List<VolumeDetailVO> shareDetails = _volDetailDao.findShareDetailByLike(volumeId);
        	List<Long> vmIds = new ArrayList<Long>();
        	if(vmId != null){
        		vmIds.add(vmId);
        	}
        	if(!shareDetails.isEmpty()){
        		for(VolumeDetailVO v : shareDetails){
        			vmIds.add(Long.valueOf(v.getValue()));
        		}
        	}
        	...
        	if(!vmIds.isEmpty()){
        		String err = "";
        		for(Long id : vmIds){
        			UserVmVO temp = _userVmDao.findById(id);
        			if(temp != null && temp.getState() != State.Stopped){
        				err = err + temp.getDisplayName() + "|";
        			}
        		}
        		...
        		if(!err.equals("")){
        			...
        			throw new InvalidParameterValueException(
                            "Please specify these virtual machines were stopped:" + err);
        		}
        	}
        }
        ...
        //add a record to volumedetail table
        VolumeDetailVO vol = _volumeDetailsDao.findDetail(volumeId, "isshared");
    	if(vol == null){
    		String name = "isshared";
    		Map<String, String> details = new HashMap<String, String>();
    		if(isShared){
    			details.put(name, "enabled");
    		}else{
    			details.put(name, "disabled");
    		}
    		
    		_volumeDetailsDao.persist(volumeId, details);
    	}
        ...
        if (createVolumeOnBackend) {
            if (volume.getState().equals(Volume.State.Allocated)
                ...
                try {
                	...
                	//it is failed to create disk,delete record whit volumedetail table
                	VolumeDetailVO voldtl = _volumeDetailsDao.findDetail(volumeId, "isshared");
                	if(vol != null){
                		String name = "isshared";
                		_volumeDetailsDao.removeDetails(volumeId, name);
                	}
...
//            boolean moveVolumeNeeded = needMoveVolume(rootVolumeOfVm, volumeOnPrimaryStorage);
            ...
            if(isShared){
            	...
            	moveVolumeNeeded = needMoveVolume(rootVolumeOfVm, volumeOnPrimaryStorage);
            }
        ...
        //if the vmdisk is shared
        newVol = sendAttachVolumeCommand(vm, newVol, deviceId, isShared);
        return newVol;
    }
    ...
    public Volume detachVolumeFromVM(DetachVolumeCmd cmmd) {
        ...
        if ((cmmd.getId() == null && cmmd.getDeviceId() == null && cmmd.getVirtualMachineId() == null)
            ...
            throw new InvalidParameterValueException(
                    "Please provide either a volume id, or a tuple(device id, instance id), or a tuple(volume id, instance id)");
        }
        ...
        if(_volumeDetailsDao.isSharedDisk(volume.getId()) == true && vm.getState() != State.Stopped){
        	throw new InvalidParameterValueException("Please specify the virtual machines was stopped.");
        }
        ...
        if (!sendCommand || (answer != null && answer.getResult())) {
        	if(vmId.longValue() == volume.getInstanceId().longValue()){
        		List<VolumeDetailVO> shareVols = _volumeDetailsDao.findShareDetailByLike(volumeId);
        		int num = (shareVols == null ? 0 : shareVols.size());
        		
        		if(num > 1){
        			String newVmId = null;
        			for(VolumeDetailVO v : shareVols){
        				if(!v.getValue().equals(String.valueOf(vmId))){
        					newVmId = v.getValue();
        					break;
        				}
        			}
        			...
        			if(newVmId != null){
        				...
        				newVol.setInstanceId(Long.valueOf(newVmId));
        				_volsDao.update(volumeId, newVol);
        			}
        			
        			_volumeDetailsDao.removeDetails(volumeId, "instance-" + vmId);
        		}else if (num == 1){
...
//        }
        ...
        //by xwl
        //find all shared disk
        List<VolumeDetailVO> volumesForVD = _volumeDetailsDao.findDetailByVmId(String.valueOf(vmId));
        for(VolumeDetailVO vd : volumesForVD){
        	VolumeVO temp = _volsDao.findById(vd.getVolumeId());
        	if(temp != null && !volumesForVm.contains(temp)){
        		volumesForVm.add(temp);
        	}
        }
        
        //by xwl
        //deal with shared disk
        for (VolumeVO vol : volumesForVm) {
            if (vol.getVolumeType().equals(Type.ROOT)) {
                // Destroy volume if not already destroyed
                boolean volumeAlreadyDestroyed = (vol.getState() == Volume.State.Destroy ||
                ...
                long volumeId = vol.getId();
                
                if(vmId == vol.getInstanceId().longValue()){
            		List<VolumeDetailVO> shareVols = _volumeDetailsDao.findShareDetailByLike(volumeId);
            		int num = (shareVols == null ? 0 : shareVols.size());
            		...
            		if(num > 1){
            			...
            			_volsDao.detachVolume(volumeId);
            			_volumeDetailsDao.removeDetails(volumeId, "instance-" + vmId);
            		}else{
            			_volsDao.detachVolume(volumeId);
            		}
    ...
    public Volume migrateVolume(MigrateVolumeCmd cmd){
        ...
        if (vol.getFormat() == ImageFormat.LUN) {
        	throw new InvalidParameterValueException("Lun device cannot be used for volume migration.");
        }
    ...
    public void prepare(VirtualMachineProfile<? extends VirtualMachine> vm,
        ...
        List<VolumeVO> vols = _volsDao.findUsableVolumesForInstance(vm.getId());
        List<VolumeDetailVO> voldetails =  _volumeDetailsDao.findDetailByVmId(String.valueOf(vm.getId()));
        if(!voldetails.isEmpty()){
        	for(VolumeDetailVO v : voldetails){
        		VolumeVO temp = _volsDao.findById(v.getVolumeId());
    ...
    public Snapshot takeSnapshot(Long volumeId, Long policyId, Long snapshotId, Account account) throws ResourceAllocationException {
        ...
        if (volume.isShare() == true) {
            throw new InvalidParameterValueException("Creating snapshot failed due to volume:" + volumeId + " is shared.");
        }
    ...
    public Snapshot allocSnapshot(Long volumeId, Long policyId) throws ResourceAllocationException {
        ...
        if (volume.getFormat() == ImageFormat.LUN) {
        	throw new InvalidParameterValueException("Lun device cannot be used for volume snapshot.");
        }
    ...
    public String extractVolume(ExtractVolumeCmd cmd) {
        ...
        if (volume.getFormat() == ImageFormat.LUN) {
        	throw new InvalidParameterValueException("Lun device cannot be used for volume download.");
        }
