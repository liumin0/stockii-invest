...
public class VMSnapshotManagerImpl extends ManagerBase implements VMSnapshotManager, VMSnapshotService {
    ...
    @Inject VolumeDetailsDao _volumeDetailsDao;
    ...
    public VMSnapshot allocVMSnapshot(Long vmId, String vsDisplayName, String vsDescription, Boolean snapshotMemory) 
        ...
        for (VolumeVO vol : vols) {
            if (vol.getState() != Volume.State.Ready) {
                throw new InvalidParameterValueException("Creating VM snapshot failed due to volume:" + vol.getId() + " is not ready");
            }
        }
        ...
        List<VolumeDetailVO> volumeDetails = _volumeDetailsDao.findDetailByVmId(String.valueOf(vmId));
		if(volumeDetails != null && volumeDetails.size() > 0){
			throw new CloudRuntimeException("Creating vm snapshot failed due to VM:" + vmId + " has shared volume.");
		}
