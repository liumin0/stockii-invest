...
public class DeploymentPlanningManagerImpl extends ManagerBase implements DeploymentPlanningManager, Manager, Listener,
    ...
    public DeployDestination planDeployment(VirtualMachineProfile<? extends VirtualMachine> vmProfile,
        ...
        s_logger.debug("vm is : "+vmProfile.getInstanceName()+" && plan is : ["+plan.getDataCenterId()+", "+plan.getPodId()+","+plan.getClusterId()+","+plan.getHostId()+","+plan.getPoolId()+"]");
        s_logger.debug("avoids include : ["+avoids.getDataCentersToAvoid()+","+avoids.getPodsToAvoid()+","+avoids.getClustersToAvoid()+","+avoids.getHostsToAvoid()+","+avoids.getPoolsToAvoid()+"]");
        if(planner != null){
            s_logger.debug("planner can handle is : "+planner.canHandle(vmProfile, plan, avoids));
        }
        ...
        if (planner != null && planner.canHandle(vmProfile, plan, avoids)) {
            while (true) {
                ...
                if (planner instanceof DeploymentClusterPlanner) {
                	s_logger.debug("planner is start");
    ...
    protected Pair<Map<Volume, List<StoragePool>>, List<Volume>> findSuitablePoolsForVolumes(
        ...
        for (VolumeVO toBeCreated : volumesTobeCreated) {
            ...
            // be reused.
            // If the volume specifies a poolId, it means that the data volume is ready when restoreVM.
