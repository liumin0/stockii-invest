...
public class SecurityGroupManagerImpl2 extends SecurityGroupManagerImpl{
    ...
    protected Map<PortAndProto, Set<String>> generateRulesForVM(Long userVmId, SecurityRuleType type) {
        ...
        UserVmVO vm = this._userVmMgr.getVirtualMachine(userVmId.longValue());
        ...
        for (SecurityGroupVMMapVO mapVO : groupsForVm) {
            ...
            for (SecurityGroupRuleVO rule : rules) {
                ...
                if (rule.getAllowedNetworkId() != null) {
                    List<SecurityGroupJoinVO> secGroupList = this._securityGroupJoinDao.searchByIds(new Long[] { rule.getAllowedNetworkId() });

                    SecurityGroupJoinVO vo = (SecurityGroupJoinVO)secGroupList.get(0);
                    //for vmware,we send the group name nor the cidr
                    if ((vm.getHypervisorType() == Hypervisor.HypervisorType.VMware) && (vo.getProjectName() != null)) {
                        cidrs.add( Hypervisor.HypervisorType.VMware.name() + "|" + vo.getProjectName() + "_" + vo.getName());
                    } else {
