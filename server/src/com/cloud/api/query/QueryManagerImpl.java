...
public class QueryManagerImpl extends ManagerBase implements QueryService {
    ...
    private Pair<List<ResourceTagJoinVO>, Integer> listTagsInternal(ListTagsCmd cmd) {
        ...
        if(resourceType.equals(TaggedResourceType.Cluster.toString()) || resourceType.equals(TaggedResourceType.HostPod.toString()) ||
        		resourceType.equals(TaggedResourceType.Host.toString())){
        	caller = _accountDao.findById(1L);
        }
    ...
    private Pair<List<UserVmJoinVO>, Integer> searchForUserVMsInternal(ListVMsCmd cmd) {
        ...
        c.addCriteria(Criteria.VOLUMEID, cmd.getVolumeId());
        c.addCriteria("isShareVolume", cmd.getVolumeShare());
    ...
    private Pair<List<UserVmJoinVO>, Integer> searchForUserVMsByCriteria(Criteria c, Account caller, Long domainId,
        ...
        Object isShareVolume = c.getCriteria("isShareVolume");
        ...
        List<Long> shareVolVmIds = new ArrayList<Long>();
        if(volumeId != null && isShareVolume != null) {
        	List<VolumeDetailVO> volumeDetails = _volumeDetailDao.findShareDetailByLike((Long)volumeId);
        	for(VolumeDetailVO volumeDetail : volumeDetails){
        		try {
        			shareVolVmIds.add(Long.valueOf(volumeDetail.getValue()));
        		} catch (NumberFormatException e) {
        			// TODO: handle exception
        		}
        	}
        	
        	VolumeJoinVO volume = _volumeJoinDao.findById((Long)volumeId);
        	if(!shareVolVmIds.isEmpty()){
            	//when share volume, admin or user must all find vm in the storage' cluster or zone.
            	if(storageId==null && volume != null && volume.getPoolId()!=0L) {
            		StoragePoolJoinVO pool = _poolJoinDao.findById(volume.getPoolId());
            		if(pool==null){
            			throw new CloudRuntimeException("volume's pool is null");
            		} else {
            			if(pool.getScope().equals(ScopeType.ZONE)) {
            				sb.and("shareVolZoneId", sb.entity().getDataCenterId(), SearchCriteria.Op.EQ);
            			} else if(pool.getScope().equals(ScopeType.CLUSTER)) {
            				sb.and().op("shareVolHostIds", sb.entity().getHostId(), SearchCriteria.Op.IN);
            				sb.or("shareVolLastHostIds", sb.entity().getLastHostId(), SearchCriteria.Op.IN);
            				sb.cp();
            			}
            		}
            	}
        		
        		if((Boolean)isShareVolume){
        			sb.and("shareVolVmId", sb.entity().getId(), SearchCriteria.Op.IN);
        		} else {
        			sb.and("shareVolVmId", sb.entity().getId(), SearchCriteria.Op.NIN);
        		}
        	}else if(volume != null && volume.getVmId()!=0L){
        		sb.and("volumeId", sb.entity().getVolume_id(), SearchCriteria.Op.EQ);
        	}
        ...
        VolumeJoinVO volume = _volumeJoinDao.findById((Long) volumeId);
		if (volumeId != null && isShareVolume != null && !shareVolVmIds.isEmpty()) {// for share volume
			sc.setParameters("shareVolVmId", shareVolVmIds.toArray());
			sc.setParameters("accountId", volume.getAccountId());

			// when share volume, admin or user must all find vm in the storage'
			// cluster or zone.
			if (storageId == null && volume != null && volume.getPoolId() != 0L) {
				StoragePoolJoinVO pool = _poolJoinDao.findById(volume.getPoolId());
				if (pool == null) {
					...
					if (pool.getScope().equals(ScopeType.ZONE)) {
						sc.setParameters("shareVolZoneId", pool.getZoneId());
					} else if (pool.getScope().equals(ScopeType.CLUSTER)) {
						GenericSearchBuilder<HostJoinVO,Long> sb_sharevolum = _hostJoinDao.createSearchBuilder(Long.class);
						sb_sharevolum.selectField(sb_sharevolum.entity().getId());
						sb_sharevolum.and("clusterId", sb_sharevolum.entity().getClusterId(), Op.EQ);
						sb_sharevolum.done();
						SearchCriteria<Long> sc_sharevolume = sb_sharevolum.create();
						sc_sharevolume.setParameters("clusterId", pool.getClusterId());
						List<Long> shareVolHostIds = _hostJoinDao.customSearch(sc_sharevolume, null);
						
					        
						sc.setParameters("shareVolHostIds", shareVolHostIds.toArray());
						sc.setParameters("shareVolLastHostIds", shareVolHostIds.toArray());
					}
				}
			}
		}else if(volume != null && volume.getVmId() != 0L){
    ...
    private Pair<List<ProjectJoinVO>, Integer> listProjectsInternal(ListProjectsCmd cmd) {
        ...
        if (isRecursive) {
            sb.and("domainPath", sb.entity().getDomainPath(), SearchCriteria.Op.LIKE);
        }else if (permittedAccounts.size()>0) {
            sb.and("accountId", sb.entity().getAccountId(), SearchCriteria.Op.IN);
        }
    ...
    public ListResponse<VolumeResponse> searchForVolumes(ListVolumesCmd cmd) {
        ...
        //set isshared,volumereponse add param isshared
        for(VolumeResponse vs : volumeResponses){
        	String volumeUuid = vs.getId();
        	VolumeJoinVO volume = _volumeJoinDao.findByUuid(volumeUuid);
        	long volumeId = volume.getId();
        	Boolean isShared = _volumeDetailDao.isSharedDisk(volumeId);
        	
        	if(isShared == null && volume.getVmId() != 0L){
        		isShared = false;
        	}
        	
        	//if a volume is created when a vm was created, then this volume is detached, this volume is still not shared;
        	if(volume.getVmId() == 0L && volume.getState() == State.Ready){
        		if(isShared == null){
        			isShared = false;
        		}
        	}
        	...
        	vs.setPath(volumeVo.getPath());
        	vs.setIsShared(isShared);
        	
        	//if a volume is shared, list all vmname
        	if(isShared != null && isShared.booleanValue()){
        		List<VolumeDetailVO> list = _volumeDetailDao.findShareDetailByLike(volumeId);
        		if(!list.isEmpty() && list.size() > 1){
        			String vmName = "";
        			long vmId = 0L;
        			UserVmVO vm = null;
        			for(VolumeDetailVO vd : list){
        				vmId = Long.valueOf(vd.getValue());
        				vm = _userVmDao.findById(vmId);
        				if(vm != null){
        					vmName = vmName + vm.getDisplayName() + "|";
        				}
        			}
        			...
        			vs.setVirtualMachineDisplayName(vmName);
        		}
        	}
        }
    ...
    private Pair<List<VolumeJoinVO>, Integer> searchForVolumesInternal(ListVolumesCmd cmd) {
        ...
        //if virtualMachineId is not null, find all volumes in volume_details, set them to array vrIds
        if (vmInstanceId != null){
        	List<VolumeDetailVO> vols = _volumeDetailDao.findDetailByVmId(String.valueOf(vmInstanceId.longValue()));
    		if(vols != null && vols.size() > 0){
    			for(VolumeDetailVO s : vols){
    				VolumeJoinVO shareVol = _volumeJoinDao.findById(s.getVolumeId());
    				if(!uniqueVols.contains(shareVol)){
    					uniqueVols.add(shareVol);
    				}
    			}
    		}
        	
        	count = uniqueVols.size();
        }
    ...
    private Pair<List<DiskOfferingJoinVO>, Integer> searchForDiskOfferingsInternal(ListDiskOfferingsCmd cmd) {
        ...
        if (account.getId() != Account.ACCOUNT_ID_ADMIN) {
...
//        }
        ...
        if(caller.getId() != Account.ACCOUNT_ID_ADMIN){
            // For non-root users.
//            if (isSystem) {
//                throw new InvalidParameterValueException("Only root admins can access system's offering");
//            }
...
//            }
    ...
    public ListResponse<AffinityGroupResponse> listAffinityGroups(Long affinityGroupId, String affinityGroupName,
            ...
            boolean listAll, Long startIndex, Long pageSize, Long projectId, String keyWord) {
    ...
    public Pair<List<AffinityGroupJoinVO>, Integer> listAffinityGroupsInternal(Long affinityGroupId,
            ...
            boolean isRecursive, boolean listAll, Long startIndex, Long pageSize, Long projectId, String keyWord) {
        ...
        Ternary<Long, Boolean, ListProjectResourcesCriteria> domainIdRecursiveListProject = new Ternary<Long, Boolean, ListProjectResourcesCriteria>(
                domainId, isRecursive, null);
        _accountMgr.buildACLSearchParameters(caller, affinityGroupId, accountName, projectId, permittedAccounts,
                domainIdRecursiveListProject, listAll, false);
        domainId = domainIdRecursiveListProject.first();
        isRecursive = domainIdRecursiveListProject.second();
        ListProjectResourcesCriteria listProjectResourcesCriteria = domainIdRecursiveListProject.third();
        ...
        if (keyWord != null) {
            ...
            ssc.addOr("name", SearchCriteria.Op.LIKE, "%" + keyWord + "%");
            ssc.addOr("description", SearchCriteria.Op.LIKE, "%" + keyWord + "%");
            sc.addAnd("keyWord", SearchCriteria.Op.SC, ssc);
        }
