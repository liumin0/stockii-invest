...
public class UserVmJoinDaoImpl extends GenericDaoBase<UserVmJoinVO, Long> implements UserVmJoinDao {
    ...
    public UserVmResponse newUserVmResponse(String objectName, UserVmJoinVO userVm, EnumSet<VMDetails> details, Account caller) {
        ...
        if (details.contains(VMDetails.all) || details.contains(VMDetails.stats)) {
            ...
            if (userVm.getState() == VirtualMachine.State.Running && vmStats != null) {
            	double cpuUses = vmStats.getCPUUtilization();
            	if(cpuUses != 0d && userVm.getHypervisorType() == HypervisorType.VMware){
            		cpuUses = cpuUses / (userVm.getCpu() * userVm.getSpeed()) * 100;
            	}
//                userVmResponse.setCpuUsed(new DecimalFormat("#.##").format(vmStats.getCPUUtilization()) + "%");
                userVmResponse.setCpuUsed(new DecimalFormat("#.##").format(cpuUses) + "%");
                
                double memoryUsed = vmStats.getMemoryUsage();
                if(memoryUsed != 0d && userVm.getHypervisorType() == HypervisorType.VMware){
                	memoryUsed = memoryUsed / userVm.getRamSize() * 100;
                }else if(userVm.getHypervisorType() == HypervisorType.XenServer){
                	memoryUsed = (userVm.getRamSize() * 1024 - memoryUsed) / (userVm.getRamSize() * 1024) * 100;
                }
                userVmResponse.setMemoryUsed(new DecimalFormat("#.##").format(memoryUsed) + "%");
        ...
        if (userVm.getDetailName() != null && (userVm.getDetailName().equalsIgnoreCase(VmDetailConstants.HYPERVISOR_TOOLS_VERSION)
        			|| userVm.getDetailName().contains("Time"))){
        ...
        if(userVm.getHypervisorType() == HypervisorType.VMware){
        	UserVmDetailVO detail = _userVmDetailsDao.findDetail(userVm.getId(), VmDetailConstants.NIC_ADAPTER);
            if(detail != null){
            	userVmResponse.setNicAdapter(detail.getValue());
            } else {
            	userVmResponse.setNicAdapter("E1000");
            }
        }
    ...
    public UserVmResponse setUserVmResponse(UserVmResponse userVmData, UserVmJoinVO uvo) {
        ...
        if(uvo.getDetailName() != null && uvo.getDetailName().contains("Time")){
            Map<String, String> resourceDetails = userVmData.getDetails();
            if (resourceDetails == null) {
            	resourceDetails = new HashMap<String, String>();
            }
            resourceDetails.put(uvo.getDetailName(), uvo.getDetailValue());
            userVmData.setDetails(resourceDetails);
        }
